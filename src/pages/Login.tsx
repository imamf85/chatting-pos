...
export default function Login() {
  const { signIn, signOut, user, profile, loading, isDemoMode, error } = useAuth();
  const navigate = useNavigate();
  const { theme, toggleTheme } = useTheme();

  // Redirect ke /order jika sudah login dan punya profile
  useEffect(() => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-emerald-100 via-emerald-50 to-white dark:from-gray-900 dark:via-gray-900 dark:to-gray-800">
        <div className="flex flex-col items-center gap-3">
          ...
          <div className="text-gray-500 dark:text-gray-400 font-medium">Memuat...</div>
        </div>
      </div>
    );
  }
  ...
  // User sudah login tapi email tidak terdaftar
  const isLoggedInButNotAllowed = user && !profile && error;

  // OAuth error (failed before session was created)
  const hasOAuthError = !user && error;
          ...
          {hasOAuthError && (
            <div className="bg-red-50 dark:bg-red-900/20 border-2 border-red-200 dark:border-red-700 rounded-2xl p-4 mb-6">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div className="text-red-800 dark:text-red-300 text-sm font-semibold">
                  Login Gagal
                </div>
              </div>
          ...
          {isLoggedInButNotAllowed ? (
            <>
              ...
              <button
                ...
                className="w-full py-4 px-4 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 font-semibold rounded-2xl
